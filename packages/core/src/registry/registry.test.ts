...
import { CheckRegistry, CheckNotFoundError, CheckLoadError } from './registry.js';
...
describe('registry', () => {
  ...
  it('throws CheckNotFoundError and CheckLoadError', () => {
    ...
    const loadErr = new CheckLoadError('msg', { requestedId: asCheckId('foo'), moduleName: 'bar', message: 'err' });
  ...
  it('validates check exports through a real mock module', async () => {
    ...
    writeMockPkg('check-bad3', `export default { id: "a", category: "lint", defaultTier: "nope" };`); // bad defaultTier
    writeMockPkg('check-bad4', `export default { id: "a", category: "lint", defaultTier: "fast", detect: 1 };`); // bad detect function
    ...
    const getFail = (id: string) => registry.loadFailures().find(f => f.requestedId === asCheckId(id))!.message;
