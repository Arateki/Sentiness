...
sentiness install-skill --agent=<claude-code|claude-code-skill|codex|gemini|all>
...
export interface AgentAdapter {
  readonly agent: AgentName;
  readonly targetFile: string;  // e.g. 'CLAUDE.md' or '.claude/skills/sentiness/SKILL.md'
...
export const TEMPLATE_VERSION = '1.1' as const;  // bump on any skill-template.md content change
```

**Implementation notes:**
- Template is a markdown file with `{{placeholders}}` for paths. `renderSkill` substitutes them.
- The rendered output is what gets embedded between the `sentiness:start` and `sentiness:end` HTML-comment markers in CLAUDE.md / AGENTS.md / GEMINI.md.
- **Never write the literal marker comments in this document.** `install-skill` replaces everything between the first marker pair it finds in the target file — including marker text quoted inside documentation. Running `install-skill --agent=claude-code` against this repository once injected the rendered template into the middle of this section (fixed 2026-06-09). Refer to the markers by name only.
- Each rendered output starts with an HTML comment line `generated by @sentiness/adapters v<TEMPLATE_VERSION>; do not edit between markers`.
...
**Behavior:**
- Generates / updates `CLAUDE.md` at the project root with a section delimited by the `sentiness:start` / `sentiness:end` HTML-comment markers (literal text intentionally not written here — see T6.4) containing the standard skill text.
...
Same pattern as T6.1. Target file: `AGENTS.md`. Same `sentiness:start` / `sentiness:end` HTML-comment markers. Codex respects this file via its hierarchical discovery.
...
#### T6.5 — Adapter: Claude Code skill

**Depends on:** T0.1, T0.2, T6.4
**Parallel-safe with:** T6.1, T6.2, T6.3

**Files:**
- `packages/adapters/src/claude-code.ts` (exports `claudeCodeSkillAdapter` alongside `claudeCodeAdapter`)
- `packages/adapters/src/claude-code.test.ts`

**Behavior:**
- Agent name: `claude-code-skill`. Target file: `.claude/skills/sentiness/SKILL.md` (relative to project root; parent directories created as needed).
